...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("select p from Product p where p.addedDate <= :addedDate ",Product.class).setParameter("addedDate", cal.getTime()).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery("SELECT NEW cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) FROM Product p LEFT JOIN p.categories c ON c.name= 'Kitchen' order by p.name " , ProductAndCategory.class).getResultList();
